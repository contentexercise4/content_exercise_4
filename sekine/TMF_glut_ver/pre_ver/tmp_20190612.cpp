...
void prosessingOfMoveOBJ(int *ver, GLuint *vertexbuffer, std::vector<glm::vec3>& vertices, std::vector<glm::vec3>& normals, std::vector<glm::vec2>& uvs, std::string OBJFile, GLuint *uvbuffer, GLuint *VertexArrayID) {
	...
	//else if(OBJFile == "pigfbx.obj"){
	else if (OBJFile == "lowpig.obj") {
		//printf("aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa");
		for (int i = 0; i < vertices.size(); i++) {
			//vertices[i].x += 1;
			//vertices[i].y += 0.001;
...
int main() {
	...
	// ウィンドウ生成
	//GLFWwindow* window = glfwCreateWindow(1960, 1080, "OpenGL Simple", glfwGetPrimaryMonitor(), NULL);
	//GLFWwindow* window = glfwCreateWindow(3840, 2160, "OpenGL Simple", glfwGetPrimaryMonitor(), NULL);
	GLFWwindow* window = glfwCreateWindow(11520, 4320, "OpenGL Simple", glfwGetPrimaryMonitor(), NULL);
