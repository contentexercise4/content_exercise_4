...
typedef BOOL (*SETRENDERINGPARAMETERSPTR)(int, int, int, int, BOOL, BOOL, float, float, float);
...
static void dimencoInit(void){
	...
	HMODULE hOpenGLDll = GetModuleHandle("opengl32.dll");

	SignalZBufferAvailable    = (SIGNALZBUFFERAVAILABLEPTR)     GetProcAddress(hOpenGLDll, "SignalZBufferAvailable");
	SignalBackgroundAvailable = (SIGNALBACKGROUNDAVAILABLEPTR)  GetProcAddress(hOpenGLDll, "SignalBackgroundAvailable");
	Enable3DMode              = (ENABLE3DMODEPTR)               GetProcAddress(hOpenGLDll, "Enable3DMode");
	SetRenderingParameters    = (SETRENDERINGPARAMETERSPTR)     GetProcAddress(hOpenGLDll, "SetRenderingParameters");
	SelectFormat              = (SELECTFORMATPTR)               GetProcAddress(hOpenGLDll, "SelectFormat");
	SetApplicationResolution  = (SETAPPLICATIONRESOLUTIONPTR)   GetProcAddress(hOpenGLDll, "SetApplicationResolution");
	SetInputResolution        = (SETINPUTRESOLUTIONPTR)         GetProcAddress(hOpenGLDll, "SetInputResolution");
	SetVideoWallBezelSize	    = (SETVIDEOWALLBEZELSIZEPTR)      GetProcAddress(hOpenGLDll, "SetVideoWallBezelSize");
...
#define FORMAT_2D_PLUS_DEPTH   0   // This is the default if SelectFormat is not called
#define FORMAT_DECLIPSE        1

	if (API_Available) SelectFormat(FORMAT_DECLIPSE);
	//assert(SetApplicationResolution(1920,1080));
	//assert(SetInputResolution(800,600));
...
static void dimencoInitState(void){
	...
	if (API_Available) SetRenderingParameters(
						64,      // factor
...
static void dimencoSetBackgroundState(void){
	...
	if (API_Available) SignalBackgroundAvailable(TRUE, TRUE);
}
...
static void dimencoSetZBufState(void){
	...
	if (API_Available) SignalZBufferAvailable();
	if (API_Available) glClear(GL_DEPTH_BUFFER_BIT);
}
