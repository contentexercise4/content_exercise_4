...
void moveOBJ() {
	...
	if (abs(position.x - 12.0) < 5 && abs(position.z - (-27.5)) < 5) {
		if (appleFlag[1]) {
			allSeStop();
			soundPlay(seGetItem, &sseGetItem, false, volume_GetItem);
			appleFlag[1] = false;
	...
	if (abs(position.x - 3.0) < 5 && abs(position.z - (-27.5)) < 5) {
		if (appleFlag[2]) {
			allSeStop();
			soundPlay(seGetItem, &sseGetItem, false, volume_GetItem);
			appleFlag[2] = false;
