...
void DirectionOfPig() {
	for (int i = 0; i < 4; i++) {
		...
		if (!gameEnd) {
			...
			//ブタがエリア内だけ追いかける
			if (i == 0 && disFromPig2 < disOfNoticePig) {
				if (position.x > -7 && position.z > 4) {
					areafind = true;
				}
				else areafind = false;
			}
			else if (i == 1 && disFromPig2 < disOfNoticePig) {
				if (position.x > 9 && position.z < -4) {
					areafind = true;
				}
				else areafind = false;
			}
			else if (i == 2 && disFromPig2 < disOfNoticePig) {
				if (position.x < 9 && position.z < -4) {
					areafind = true;
				}
				else areafind = false;
			}
			else if (i == 3 && disFromPig2 < disOfNoticePig) {
...
//--------------------------------------------------------------------------------------------------------
void decideVolumeOfPigCry(double disFromPig2) {
	if (disFromPig2 >= disOfNoticePig) {
		volume_PIG = 0.0f;
	}
	else {
		//volume_PIG = 1.0f / ((float)disFromPig) + 0.5f;
		volume_PIG = 1.3f - ((float)disFromPig2) / 10.0;
