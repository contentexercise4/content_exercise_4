...
void Initialize() {
	...
	Texture[28].Load("bmp/gameover3.bmp");
	Texture[29].Load("bmp/gameclear3.bmp");
	Texture[30].Load("bmp/timeup3.bmp");
...
void PlayMode() {
	...
	AppleCheck();       //リンゴアラート
		//リンゴがなくなってたらリザルトに
	if (!appleFlag[0] && !appleFlag[1] && !appleFlag[2] && !appleFlag[3]) {
		GameClear();
		//ModeSelect = 2;
		//sound_check = 2;
	}
	...
	if (dbg_counter == 100) {
		...
		//	std::cout << "volume_PIG = " << volume_PIG << std::endl;
			//dbg_counter = 0;
			//std::cout << "keyboad(" << objflag.xposDec << objflag.xposInc << objflag.keyUP << objflag.keyDOWN << objflag.hitRIGHT << objflag.hitLEFT << ")\n";
	}
	//dbg_counter++;
}
...
void selectDegreeOfDifficulty(int dod) {
	if (selectEnviroment == MYPC) {
		...
		if (dod == EASY) {
			setPigSpeed(0.02*Bigger);
			change_obj_speed = 0;
			accessPig = false;
		}
		else if (dod == NORMAL) {
			setPigSpeed(0.025*Bigger);
			change_obj_speed = 0;
			accessPig = true;
		}
		else if (dod == HARD) {
	...
	if (selectEnviroment == MYPC) {
		if (applecount == 4) {											// 取得すべきリンゴはあと4つ
			...
			if (!changePigSpeedFlag[3] && digreeOfDifficulty == HARD) {	// まだ速さを変えていないならば(HARDのみ)
				...
				setPigSpeed(0.006*Bigger);										// HARDのときのみ
			...
			if (!changePigSpeedFlag[2] && digreeOfDifficulty == HARD) {	// まだ速さを変えていないならば(HARDのみ)
				...
				setPigSpeed(0.007*Bigger);										// HARDのときのみ
			...
			if (!changePigSpeedFlag[1] && digreeOfDifficulty == HARD) {	// まだ速さを変えていないならば(HARDのみ)
				...
				setPigSpeed(0.008*Bigger);										// HARDのときのみ
				//setPigSpeed(0.05);											// 初速
			}
		}
...
void AppleCheck() {
	...
	if (abs(position.x - 10.0) < 11 && abs(position.z - (-27.5)) < 11) {
		if (appleFlag[1]) {
			glBindTexture(GL_TEXTURE_2D, Texture[27].ID);				// テクスチャをバインド (追加 bmp)
			ACflag = true;
		}
	...
	if (abs(position.x - 5.0) < 11 && abs(position.z - (-27.5)) < 11) {
		if (appleFlag[2]) {
			glBindTexture(GL_TEXTURE_2D, Texture[27].ID);				// テクスチャをバインド (追加 bmp)
			ACflag = true;
		}
