...
void PlayMode() {
	...
	if (defTime > 180) {//ゲーム終了
		...
		glEnable(GL_TEXTURE_2D);
		//　テクスチャをバインド (追加 bmp)
		glBindTexture(GL_TEXTURE_2D, Texture[24].ID);
		glPushMatrix();
		...
		glEnable(GL_TEXTURE_2D);
		//　テクスチャをバインド (追加 bmp)
		glBindTexture(GL_TEXTURE_2D, Texture[25].ID);
		glPushMatrix();
		ObjMoveRoll_Camera(horizontalAngle, verticalAngle, 0);
		//glTranslatef(-11.0, 1.05, -15.6);
		//glRotatef(90.0f,0.0f,1.0f,0.0f);
	...
	if (applecount == 4) {
		...
		glBindTexture(GL_TEXTURE_2D, Texture[11].ID);
	}
	else if (applecount == 1) {
		glBindTexture(GL_TEXTURE_2D, Texture[12].ID);
	}
	else if (applecount == 0) {
		glBindTexture(GL_TEXTURE_2D, Texture[13].ID);
	}
	glPushMatrix();
	...
	dif_x = position.x - pigx;
	dif_z = position.z - pigz;
	...
	if (disFromPig < 20) {
		...
		/*for (int i = 0; i < vertices.size(); i++) {
			...
			if (objhit(vertices[i].x, vertices[i].z, cube) == true) {
				std::cout << "xxxxxhit\n";
				for (int i = 0; i < vertices.size(); i++) {
					vertices[i].x -= obj_xspeed;
				}
...
void decideVolumeOfPigCry(double disFromPig) {
	if (disFromPig >= 20) {
		...
		volume_PIG = ((float)20 - (float)disFromPig) * (float)0.06;
		std::cout << "volume = " << volume_PIG << '\n';
		soundPlay(sePig, &ssePig, false, volume_PIG);
	}
	//std::cout << "dis="<<disFromPig << std::endl;
}
