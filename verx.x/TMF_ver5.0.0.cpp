...
void Initialize() {
	...
	Texture[3].Load("bmp/START_back2.bmp");
	//Texture[3].Load("bmp/start.bmp");
	//Texture[3].Load("bmp/start_clear.bmp");
	Texture[4].Load("bmp/black.bmp");
	Texture[5].Load("bmp/enter2.BMP");
	//Texture[5].Load("bmp/enter_clear.BMP");
...
void GameReset() {
	...
	pigx[0] = -2 + (double)test*2.0;
	pigx[1] = 20;
	pigx[2] = -2 - (double)test*2.0;
	pigx[3] = -14 - (double)test*0.9;
	pigz[0] = 26;
	pigz[1] = -2.5 - (double)test*2.0;
	pigz[2] = -26.5;
	pigz[3] = 16.5;
	pigy[0] = -1.0;
	pigy[1] = -1.0;
	...
	alDeleteBuffers(soundCnt.sePigCnt - 1, &sePig);
	alDeleteSources(soundCnt.sePigCnt - 1, &ssePig);
	soundCnt.sePigCnt = 0;

	for (int i = 0; i < 180; i++) {
		controllerPigCry[i] = false;
	}
...
void TimeLimit() {
	...
	if (defTime > 180 && !endflag.gameoverFlag && !endflag.gameclearFlag) {//ゲーム終了
		TimeUp();
		//ModeSelect = 2;
		//sound_check = 2;
	}
		...
		if (!gameEnd) {
			...
			*/
			...
			if (accessPig == true && areafind == true) {	//ブタの接近フラグがtrue(難易度EASYのみ) かつ, ブタと自分の距離がdisOfNoticePig以下
				...
				if (disFromPig2 < disOfNoticePig && areafind == true) {
					decideVolumeOfPigCry(disFromPig2);
				}
...
//--------------------------------------------------------------------------------------------------------
void GameOver() {
	if (!endflag.gameoverFlag) {
		allSeStop();
		soundStop(&bgmPlay, &sbgmPlay);
		soundStop(&sePig, &ssePig);
...
void soundPlay(ALuint buf, ALuint *source, bool loop, float volume) {
	...
	if (loop) {												// 再生する音声がLOOP再生するBGMならば
		...
		//		//soundStop(source);
		//		if (checkSoundCnt(source)) {
		//			alDeleteBuffers(1, source);								// 一度不要なバッファを削除してから	
		//			DecSoundCnt(source);									// デクリメント
		//		}
		//		//alDeleteBuffers(1, source);								// 一度不要なバッファを削除してから	
		//		//DecSoundCnt(source);										// デクリメント
		//		alGetError();
...
void soundStop(ALuint *buf, ALuint *source) {
	...
	//source = 0;
	//alDeleteBuffers(1, buf);							// 音データのバッファの削除
	//source = NULL;										// 音データの初期化
	//buf == NULL;										// 音データの初期化
}
...
void allSoundStop() {
	soundStop(&sePig, &ssePig);
	soundStop(&seGetItem, &sseGetItem);
	soundStop(&seCollisionWall, &sseCollisionWall);
	soundStop(&bgmStart, &sbgmStart);
	soundStop(&bgmPlay, &sbgmPlay);
	soundStop(&bgmResult, &sbgmResult);
	//std::cout << '\n';
}
...
void IncSoundCnt(ALuint *source) {
	if (source == &sseCollisionWall) {
		soundCnt.seCollisionWallCnt++;
	}
	if (source == &sseGetItem) {
		soundCnt.seGetItemCnt++;
	}
	if (source == &ssePig) {
		soundCnt.sePigCnt++;
	}
	if (source == &sseGameover) {
		soundCnt.seGameoverCnt++;
	}
	if (source == &sseGameclear) {
		soundCnt.seGameclearCnt++;
	}
	if (source == &sseTimeup) {
		soundCnt.seTimeupCnt++;
	}
	if (source == &sbgmStart) {
		soundCnt.bgmStartCnt++;
	}
	if (source == &sbgmPlay) {
		soundCnt.bgmPlayCnt++;
	}
	if (source == &sbgmResult) {
		soundCnt.bgmResultCnt++;
	}
}
...
//--------------------------------------------------------------------------------------------------------
void InitSoundcnt() {
	soundCnt.bgmStartCnt = 0;
	soundCnt.bgmPlayCnt = 0;
	soundCnt.bgmResultCnt = 0;
	soundCnt.sePigCnt = 0;
	soundCnt.seCollisionWallCnt = 0;
	soundCnt.seGetItemCnt = 0;
	soundCnt.seGameoverCnt = 0;
	soundCnt.seGameclearCnt = 0;
	soundCnt.seTimeupCnt = 0;
}
...
//--------------------------------------------------------------------------------------------------------
void PrintSoundCnt() {
	std::cout << "Start     = " << soundCnt.bgmStartCnt << std::endl;
	std::cout << "Play      = " << soundCnt.bgmPlayCnt << std::endl;
	std::cout << "Result    = " << soundCnt.bgmResultCnt << std::endl;
	std::cout << "PIg       = " << soundCnt.sePigCnt << std::endl;
	std::cout << "Wall      = " << soundCnt.seCollisionWallCnt << std::endl;
	std::cout << "Item      = " << soundCnt.seGetItemCnt << std::endl;
	std::cout << "Gameover  = " << soundCnt.seGameoverCnt << std::endl;
	std::cout << "Gameclear = " << soundCnt.seGameclearCnt << std::endl;
	std::cout << "Timeup    = " << soundCnt.seTimeupCnt << std::endl;
	std::cout << std::endl;
}
...
bool checkSoundCnt(ALuint *source) {
	bool flag = false;
	if (source == &sseCollisionWall) {
		if (soundCnt.seCollisionWallCnt >= 0) {
			flag = true;
		}
	}
	if (source == &sseGetItem) {
		if (soundCnt.seGetItemCnt >= 0) {
			flag = true;
		}
	}
	if (source == &ssePig) {
		if (soundCnt.sePigCnt >= 0) {
			flag = true;
		}
	}
	if (source == &sseGameover) {
		if (soundCnt.seGameoverCnt >= 0) {
			flag = true;
		}
	}
	if (source == &sseGameclear) {
		if (soundCnt.seGameclearCnt >= 0) {
			flag = true;
		}
	}
	if (source == &sseTimeup) {
		if (soundCnt.seTimeupCnt >= 0) {
			flag = true;
		}
	}
	if (source == &sbgmStart) {
		if (soundCnt.bgmStartCnt >= 0) {
			flag = true;
		}
	}
	if (source == &sbgmPlay) {
		if (soundCnt.bgmPlayCnt >= 0) {
			flag = true;
		}
	}
	if (source == &sbgmResult) {
		if (soundCnt.bgmResultCnt >= 0) {
			flag = true;
		}
	}
	return flag;
...
void decideVolumeOfPigCry(double disFromPig) {
	if (disFromPig >= disOfNoticePig) {
		...
		volume_PIG = 1.2f - ((float)disFromPig2) / 10.0;
		if (!controllerPigCry[(int)defTime]) {
			//std::cout << "PLAY_PIG = " << (int)defTime << std::endl;
			//std::cout << "Volume   = " << volume_PIG << std::endl;
			controllerPigCry[(int)defTime] = true;
