...
void Initialize() {
	...
	FoV = 45.0f;				//なんか難しいやつ
	hspeed = 1.0f;
	disOfNoticePig = 12;		//主人公と豚との距離がdisOfNoticePig以下になるとブタが近づきながら鳴き声を上げる

	std::cout << "Enviroment = " << selectEnviroment << std::endl;
	...
	// 描画に使うobjの初期化
	//-----------------------------------------------------------------------
	OBJFile[0].LoadFile("obj/Newwall.obj");
	...
	Texture[24].Load("bmp/time/finish.bmp");
	Texture[25].Load("bmp/time/10sec.bmp");
	//Texture[24].Load("bmp/time/3sec.bmp");
	//Texture[25].Load("bmp/time/3sec.bmp");
	//Texture[25].Load("bmp/time/finish.bmp");

	//Texture[26].Load("bmp/time/3_00.bmp");

	//ここまではいける


	//Texture[27].Load("bmp/time/3sec.bmp");
...
void moveOBJ() {
	...
	*/
	...
	//リンゴのあたり判定
	if (abs(position.x - 28) < 5 && abs(position.z - (26)) < 5) {
		if (appleFlag[0]) {
	...
	if (abs(position.x - 10.0) < 5 && abs(position.z - (-27.5)) < 5) {
		if (appleFlag[1]) {
			soundPlay(seGetItem, &sseGetItem, false, volume_GetItem);
			appleFlag[1] = false;
		}
	...
	if (abs(position.x - 5.0) < 5 && abs(position.z - (-27.5)) < 5) {
		if (appleFlag[2]) {
			soundPlay(seGetItem, &sseGetItem, false, volume_GetItem);
			appleFlag[2] = false;
		}
	...
	if (abs(position.x - (-18.5)) < 5 && abs(position.z - 0) < 5) {
		if (appleFlag[3]) {
			soundPlay(seGetItem, &sseGetItem, false, volume_GetItem);
			appleFlag[3] = false;
		}
...
void NumOfApple() {
	...
	if (applecount == 4) {
		...
		if (!changePigSpeedFlag[3] && digreeOfDifficulty == HARD) {	// まだ速さを変えていないならば(HARDのみ)
			...
			//if (selectEnviroment == MYPC) {
			//	setPigSpeed(0.01);									// HARDのときのみ
			//}
			//else if (selectEnviroment == DIMENCO) {
			//	setPigSpeed(0.05);									// HARDのときのみ
			//}
			setPigSpeed(0.01);										// MYPC
			//setPigSpeed(0.05);										// DIMENCO
		...
		if (!changePigSpeedFlag[2] && digreeOfDifficulty == HARD) {	// まだ速さを変えていないならば(HARDのみ)
			...
			//if (selectEnviroment == MYPC) {
			//	setPigSpeed(0.009);									// HARDのときのみ
			//}
			//else if (selectEnviroment == DIMENCO) {
			//	setPigSpeed(0.045);									// HARDのときのみ
			//}
			setPigSpeed(0.009);										// MYPC
			//setPigSpeed(0.045);										// DIMENCO
		}
	}
	else if (applecount == 1) {
		...
		if (!changePigSpeedFlag[1] && digreeOfDifficulty == HARD) {	// まだ速さを変えていないならば(HARDのみ)
			...
			//if (selectEnviroment == MYPC) {
			//	setPigSpeed(0.007);									// HARDのときのみ
			//}
			//else if (selectEnviroment == DIMENCO) {
			//	setPigSpeed(0.035);									// HARDのときのみ
			//}
			setPigSpeed(0.007);										// MYPC
			//setPigSpeed(0.035);										// DIMENCO
		}
	}
		...
		if (!changePigSpeedFlag[0] && digreeOfDifficulty == HARD) {	// まだ速さを変えていないならば(HARDのみ)
			...
			//if (selectEnviroment == MYPC) {
			//	setPigSpeed(0.005);									// HARDのときのみ
			//}
			//else if (selectEnviroment == DIMENCO) {
			//	setPigSpeed(0.025);									// HARDのときのみ
			//}
			setPigSpeed(0.005);										// MYPC
			//setPigSpeed(0.025);										// DIMENCO
		}
	}
	glPushMatrix();
...
void DirectionOfPig() {
	for (int i = 0; i < 4; i++) {
		...
		glTranslatef(pigx[i], -1.0, pigz[i]);			// ブタを移動
		...
		//std::cout << disFromPig << "\n";
		//std::cout << obj_xspeed << "\n";
		if (accessPig == true && disFromPig2 < disOfNoticePig) {				//ブタの接近フラグがtrue(難易度EASYのみ) かつ, ブタと自分の距離がdisOfNoticePig以下
			obj_xspeed = (double)(dif_x / disFromPig * obj_speed);
			obj_zspeed = (double)(dif_z / disFromPig * obj_speed);
